...
tocomp %>%
  group_by(gm) %>% 
  summarise(pval = fligner.test(I_len,I_mat,alternative="t")$p.value)
```
...
tocomp %>%
  group_by(gm) %>% 
  summarise(pval = fligner.test(IV_len,IV_mat,alternative="t")$p.value)
```
...
```{r}
bnf <- bnlearn::h2pc(databn_finite[,c(4,9:17)])
...
fitted <- bn.fit(bnf,databn_finite[,c(4,9:17)])
```
...
```{r}
bnf <- bnlearn::h2pc(databn_finite[,c(5,9:17)])
...
fitted <- bn.fit(bnf,databn_finite[,c(5,9:17)])
```
